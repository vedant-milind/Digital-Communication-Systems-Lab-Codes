...
disp('Mymbolic form information for M-ary');
disp(ass);
fprintf('\n');
 
x1=[0:M-1];
p=fskmod(ass,M,80000,8,2000000);  
sym=0:1:M-1;    
pp=fskmod(sym,M,80000,8,2000000);          
...
subplot(3,1,3);
plot(tt,m);
title('Waveform for M-ary modulation acording to symbolic information');
xlabel('Time(sec)');
ylabel('Amplitude(volt)');
...
subplot(3,1,2);
stem(ax,'linewidth',2);
title('Re-obtain symbol after M-ary demodulation ');
xlabel('n(discrete time)');
ylabel('Magnitude');
 
disp('Re-obtain symbol after M-ary demodulation ');
disp(ax);
fprintf('\n');
...
disp('Re-obtain binary information after M-ary demodulation');
disp(re_bi_in')
fprintf('\n');
